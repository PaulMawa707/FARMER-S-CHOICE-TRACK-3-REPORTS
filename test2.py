...
if group_file:
    ...
    if not group_data.empty:
        ...
        from_date = from_date.astimezone(nairobi_tz)
        to_date = to_date.astimezone(nairobi_tz)
        ...
        if st.button("Fetch Reports"):
            if from_date and to_date:
                ...
                trips_df2['Beginning'] = trips_df2['Beginning'].dt.tz_localize('UTC').dt.tz_convert('Africa/Nairobi')
                trips_df2['End'] = trips_df2['End'].dt.tz_localize('UTC').dt.tz_convert('Africa/Nairobi')

                # Convert to naive datetime (remove timezone offset)
                trips_df2['Beginning'] = trips_df2['Beginning'].dt.tz_localize(None)
                trips_df2['End'] = trips_df2['End'].dt.tz_localize(None)

                # Ensure from_date and to_date are also naive (no timezone)
                ...
                utilization = pd.pivot_table(trips_df2, values='Mileage', 
                            index='Driver', columns=['year', 'month', 'day'],
                ...
                daily_utilization = pd.pivot_table(trips_df2, values='Mileage', 
                                                        index='Driver', columns=trips_df2.Beginning.dt.day_name(),
                                                        fill_value=0.0, aggfunc=sum
                                                        )
                ...
                events_pvt = df2.pivot_table(values='Count', index='Driver', columns='Violation', fill_value=0, aggfunc='sum')
                events_pvt.reset_index(inplace=True)

                columns = ['Driver', 'Total Distance (km)']
                mileage_df = utilization[columns]
                merged_df = mileage_df.merge(events_pvt,on='Driver', how='outer' )
                
                # Add Advanced Score column
                violation_columns = merged_df.columns.difference(['Driver', 'Total Distance (km)'])  # Identify violation columns
                merged_df['Advanced Score'] = merged_df[violation_columns].sum(axis=1)
                merged_df = merged_df.sort_values(by='Advanced Score', ascending=True)
                ...
                excel_file = create_excel_file(utilization, df2, merged_df, trips_df2, group_name)
